    ...
    <table class="table table-striped">
      <thead>
        <tr>
          ...
          <th>가입방법</th>
          <th>대출한도</th>
          <th @click="sortTable('rpay_type_nm')">대출상환유형</th>
          <th @click="sortTable('lend_rate_type_nm')">대출금리유형</th>
          <th @click="sortTable('lend_rate_min')">대출금리 최저</th>
          <th @click="sortTable('lend_rate_max')">대출금리 최고</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="product in sortedProducts" :key="product.fin_prdt_cd">
          <template v-if="filter_bank(product.bank.fin_co_no)">
            ...
            <td>{{ product.infoproducts[0].rpay_type_nm }}</td>
            <td>{{ product.infoproducts[0].lend_rate_type_nm }}</td>
            <td>{{ product.infoproducts[0].lend_rate_min }}</td>
            <td>{{ product.infoproducts[0].lend_rate_max }}</td>
          </template>
        </tr>
      </tbody>
    </table>
              ...
              <table class="table table-bordered table-sm">
                ...
                <tbody>
                  <tr v-for="product in selectedProducts" :key="product.fin_prdt_cd">
                    ...
                    <td>{{ product.infoproducts[0].lend_rate_max }}</td>
                    <td>
                      <button @click="goToDetailPage(product.product_pk)" class="btn btn-info btn-sm">상세 보기</button>
                    </td>
                  </tr>
                </tbody>
              </table>
